...
# Launch photo service (no longer needed.  we use sv (runit) instead)
# nohup sh -c 'cd /home/pi/rasberry-node-frame && /usr/local/bin/nodemon app.js' > run.log &
...
printf 'Waiting for photo service to be up before launching X/Chrome'
until $(curl --output /dev/null --silent --head --fail http://localhost:8080/info); do
    printf '.'
    sleep 5
done
printf 'Looks like webserver is up!'
